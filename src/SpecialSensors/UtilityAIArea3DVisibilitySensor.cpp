...
void UtilityAIArea3DVisibilitySensor::_bind_methods() {
    ...
    ADD_PROPERTY(PropertyInfo(Variant::NODE_PATH, "visibility_volume_nodepath", PROPERTY_HINT_NONE), "set_visibility_volume_nodepath","get_visibility_volume_nodepath");
    ...
    ClassDB::bind_method(D_METHOD("set_entered_areas", "entered_areas"), &UtilityAIArea3DVisibilitySensor::set_entered_areas);
    ClassDB::bind_method(D_METHOD("get_entered_areas"), &UtilityAIArea3DVisibilitySensor::get_entered_areas);
    ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "entered_areas", PROPERTY_HINT_ARRAY_TYPE, vformat("%s/%s:%s", Variant::OBJECT, PROPERTY_HINT_RESOURCE_TYPE, "Area3D")), "set_entered_areas","get_entered_areas");
...
double UtilityAIArea3DVisibilitySensor::evaluate_sensor_value() {
    ...
    for( int i = 0; i < _entered_areas.size(); ++i ) {
        Area3D* area = godot::Object::cast_to<Area3D>(_entered_areas[i]);
        if( area == nullptr ) {
            continue;
        }
        Vector3 area_position = area->get_global_position();
        if( _do_occlusion_test ) {    
            Vector3 to = area_position;// + Vector3(0.0, 0.5, 0.0);
            if (to == zero_vector) {
                continue; 
            }
            Ref<PhysicsRayQueryParameters3D> params = godot::PhysicsRayQueryParameters3D::create(_from_vector, to, _collision_mask);
...
void UtilityAIArea3DVisibilitySensor::on_area_entered(Area3D* area ) {
    ...
    if( _entered_areas.has(area) ) {
        return;
    }   
    _entered_areas.push_back(area);
}
...
void UtilityAIArea3DVisibilitySensor::on_area_exited(Area3D* area ) {
    ...
    int index = _entered_areas.find(area);
    if( index < 0 ) {
        return;
    }
    _entered_areas.remove_at(index);
}
...
void UtilityAIArea3DVisibilitySensor::set_entered_areas( TypedArray<Area3D> entered_areas ) {
    _entered_areas = entered_areas;
}

TypedArray<Area3D> UtilityAIArea3DVisibilitySensor::get_entered_areas() const {
    return _entered_areas;
}
